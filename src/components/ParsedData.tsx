...
export function ParsedData({ snippet }: { snippet: string }) {
	try {
		...
		const renderValue = (val: unknown): import('preact').ComponentChildren => {
										...
										{val.map((item) => {
											...
											return (
												<tr className="hover:bg-ctp-surface0/30 transition-colors">
													{keys.map((k) => (
														<td className="p-2.5 border-b border-ctp-surface0 text-ctp-text last:border-b-0">{renderValue(obj[k])}</td>
													))}
												</tr>
											);
										})}
				...
				return (
					<div className="pl-3 border-l-2 border-ctp-surface0 mt-1 flex flex-col gap-1 w-full">
						{Object.entries(obj).map(([k, v]) => {
							...
							return (
								<div
									className={`mb-2 flex gap-3 items-start border-b border-ctp-surface0/30 pb-2 last:border-b-0 ${isComplex ? 'block' : ''}`}
								>
									<span className="text-ctp-overlay0 font-bold whitespace-nowrap min-w-[150px] flex items-center gap-1.5">
										{k}
										{countSuffix && <small className="text-ctp-overlay0">{countSuffix}</small>}:
									</span>
			...
			if (isHealthWord) {
				return (
					<span className={`${colorClass} font-bold uppercase text-[0.7rem] flex items-center gap-1`}>
						{lower === 'pass' || lower === 'up' || lower === 'ok' || lower === 'healthy' ? (
							<CircleCheck size={12} />
						) : (
							<CircleX size={12} />
						)}
		...
		return (
			<pre className="bg-ctp-crust p-4 rounded-lg text-xs text-ctp-overlay0 m-0 whitespace-pre-wrap border border-ctp-surface0">
