				...
				<Layout title="StatusFlare Admin - Login">
					<div className="flex items-center justify-center min-h-screen p-4">
						<div className="bg-ctp-mantle p-10 rounded-xl w-full max-w-md shadow-xl border border-ctp-surface0">
							...
							{error && (
								<div className="text-ctp-red text-sm text-center mb-4 flex items-center justify-center gap-2">
									<TriangleAlert size={16} /> {error}
								</div>
							)}

							<a
								href="/admin/login/oidc"
								className="flex items-center justify-center gap-2 w-full py-3.5 px-4 rounded-lg bg-ctp-mauve text-ctp-crust text-center no-underline font-bold text-lg border-none transition-transform hover:scale-[1.02] active:scale-[0.98]"
							>
							...
							<details className="mt-6">
								<summary className="block text-center mt-5 text-[0.75rem] text-ctp-overlay0 cursor-pointer list-none hover:text-ctp-subtext0 transition-colors uppercase tracking-widest">
								...
								<form method="POST" action="/admin/login" className="mt-4 space-y-3">
									<input
										...
										className="w-full p-3 rounded-lg border border-ctp-surface0 bg-ctp-base text-ctp-text box-border focus:ring-2 focus:ring-ctp-mauve focus:outline-none transition-all"
										required
									/>
									<button
										type="submit"
										className="w-full p-3 rounded-lg border-none bg-ctp-surface1 text-ctp-text font-bold cursor-pointer hover:bg-ctp-surface2 transition-colors flex items-center justify-center gap-2"
									>
										<Clock size={18} />
										Login
									</button>
			...
			<Layout title="StatusFlare Admin - Manage Services">
				<div className="p-5 sm:p-10 max-w-7xl mx-auto">
					<header className="flex justify-between items-center mb-10">
						...
						<a
							href="/admin/logout"
							className="flex items-center gap-2 text-ctp-overlay0 no-underline text-sm border border-ctp-surface0 px-3 py-1.5 rounded-lg hover:bg-ctp-surface0 hover:text-ctp-red transition-all group"
						>
						...
						<div className="bg-ctp-mantle p-6 rounded-xl border border-ctp-surface0 shadow-lg mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
							...
							<div className="flex items-center gap-4">
								...
								<form method="POST" action="/admin/notifications/toggle">
									...
									<button
										type="submit"
										className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition-all active:scale-95 border-none cursor-pointer ${
										...
										activeIncidents.map((i) => (
											<tr className="border-b border-ctp-surface0 last:border-b-0 hover:bg-ctp-base/50 transition-colors">
												...
												<td className="p-4 text-sm flex items-center gap-2">
													<Server size={14} className="text-ctp-overlay0" /> {i.service_name || 'System Wide'}
												</td>
												<td className="p-4 text-sm text-ctp-overlay0">
													<div className="flex items-center gap-1.5">
														<Clock size={14} /> {fmtTime(i.created_at)}
													</div>
												</td>
