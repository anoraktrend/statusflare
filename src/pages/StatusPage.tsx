...
import { Activity, ShieldCheck, TriangleAlert, ExternalLink, Clock, CircleCheck, CircleX, History, Server } from 'lucide-preact';
...
export function renderStatusPage(
	services: { name: string; url: string; icon?: string | null; history: { status: string; timestamp: string; latency_ms: number }[]; latest: { status: string; latency_ms?: number } }[],
	historicalIncidents: { name: string; status_code: number; response_snippet: string; timestamp: string }[],
				...
				<div className="max-w-4xl mx-auto px-5 py-10">
					<header className="mb-10 text-center space-y-2">
						...
						<h1 className="text-5xl font-bold tracking-tight text-ctp-text">
							Status<span className="text-ctp-mauve">Flare</span>
						</h1>
						...
						<div className="mb-12 flex flex-col gap-6">
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
								...
								<div className="bg-ctp-mantle p-6 rounded-2xl border border-ctp-surface0 shadow-lg flex flex-col justify-center">
									...
									<div className="text-3xl font-bold text-ctp-mauve">
										{system.history.length > 0
											? (system.history.reduce((a, b) => a + b.latency_ms, 0) / system.history.length).toFixed(0)
											: 0}
										ms
									</div>
								</div>
						...
						<div className="mb-12">
							...
							<div className="border border-ctp-red rounded-2xl overflow-hidden shadow-xl bg-ctp-mantle">
								{manualIncidents.map((i) => (
									<div className="p-6 border-b border-ctp-surface0 last:border-b-0 bg-ctp-red/5" key={i.id}>
					...
					<div className="border border-ctp-surface0 rounded-2xl overflow-hidden bg-ctp-mantle shadow-lg">
						{historicalIncidents.length === 0 ? (
							...
							historicalIncidents.map((incident) => (
								<div
									className="p-5 px-6 border-b border-ctp-surface0 last:border-b-0 flex justify-between items-center gap-4 hover:bg-ctp-base/30 transition-colors"
									key={incident.timestamp + incident.name}
								>
									<div className="flex items-center gap-4">
										...
										<div>
											...
											<span className="text-xs text-ctp-overlay0 font-mono tracking-tighter">
												HTTP {incident.status_code || 'Error'}: {incident.response_snippet?.slice(0, 40)}...
											</span>
										</div>
									</div>
